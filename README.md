...
- **D-Bus / keyring access from the MCP sandbox** (J-1, confirmed-fix). The default SandboxNet profile uses `--ro-bind / /` (read-only host root) and masks `/run/user` and `/tmp` with empty tmpfs mounts, blocking a v1.1.1 exploit that reached the session D-Bus socket (`/run/user/$UID/bus`) from inside the netns-blocked sandbox and read other secrets directly from the keyring, and blocking the P0-1 two-call exfil chain that wrote secrets to persistent host paths. The same masks also close the cross-call `/tmp/dbus-*` rendezvous side-channel. Residual exposure (system D-Bus at `/run/dbus`, home-directory sockets such as `~/.gnupg/S.gpg-agent` and `~/.local/share/kwalletd/`) remains under default SandboxNet — pick `isolation="full"` for a complete FS view.
